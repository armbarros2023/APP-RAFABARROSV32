...
const PatientManagementPage: React.FC = () => {
  ...
  useEffect(() => {
      localStorage.setItem(TEAM_STORAGE_KEY, JSON.stringify(therapistsList));
  }, [therapistsList]);
  ...
  useEffect(() => {
      ...
      const loadTherapists = async () => {
          try {
              ...
              if (!ignore) {
                  setSyncMessage(prev => prev || 'Nao foi possivel sincronizar tudo com o backend. Exibindo cache local.');
              }
          }
      };
      ...
      const loadStudents = async () => {
          ...
          try {
              ...
              if (user.role === 'ADMIN') {
                  ...
                  const ownBranch = therapistsList.find(therapist => therapist.id === user.id)?.branchId;
                  if (ownBranch) {
                      filters.branchId = ownBranch;
                  }
              }
              ...
              if (!ignore) {
                  setSyncMessage('Nao foi possivel sincronizar os alunos com o backend. Exibindo cache local.');
              }
          } finally {
              if (!ignore) {
  ...
  }, [user, selectedBranch?.id, therapistsList]);
  ...
  const handleDeleteStudent = (id: string) => {
      const removeStudent = async () => {
          ...
          try {
              ...
              alert('Nao foi possivel excluir este aluno agora.');
          }
      };
      ...
      const saveStudent = async () => {
          try {
              if (editingStudent) {
                  ...
                  const therapistBranchId = therapistsList.find(therapist => therapist.id === studentData.therapistId)?.branchId;
                  const branchId = selectedBranch?.id || therapistBranchId;

                  if (!branchId) {
                      alert('Selecione uma filial ou um terapeuta vinculado antes de salvar o aluno.');
                      return;
                  }
              ...
              alert('Nao foi possivel salvar este aluno na API agora.');
          }
      };
