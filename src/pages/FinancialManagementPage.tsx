...
const FinancialManagementPage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const loadTransactions = async () => {
      ...
      try {
        const data = await financialTransactionService.getAll(selectedBranch?.id ? { branchId: selectedBranch.id } : {});
        if (!ignore) {
        ...
        if (!ignore) {
          setSyncMessage('Nao foi possivel sincronizar o financeiro com a API. Exibindo fallback local quando necessario.');
        }
      } finally {
        if (!ignore) {
    ...
    const deleteTransaction = async () => {
      if(window.confirm("Tem certeza?")) {
        try {
          ...
          addToast('Nao foi possivel excluir esta transação na API.', 'error');
        }
      }
    };
    ...
    const saveTransaction = async () => {
      try {
        if (currentTransaction?.id) {
          const updatedTransaction = await financialTransactionService.update(newTransactionData.id, newTransactionData);
          setTransactions(prev => prev.map(t => t.id === newTransactionData.id ? updatedTransaction : t));
        } else {
          const createdTransaction = await financialTransactionService.create(newTransactionData);
          setTransactions(prev => [createdTransaction, ...prev]);
        }
        setShowForm(false);
        setCurrentTransaction(null);
        setSyncMessage('');
        addToast('Transação salva com sucesso.');
      } catch (error) {
        addToast('Nao foi possivel salvar esta transação na API.', 'error');
      }
    };
      ...
      {activeTab === 'cashflow' && (
          <div className="space-y-6">
              <Card title="Lançamentos Detalhados">
                {loadingTransactions ? (
                  <p className="text-slate-500 dark:text-slate-400 text-center py-4">Carregando transações...</p>
                ) : (
