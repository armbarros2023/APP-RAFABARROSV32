...
const TeamManagementPage: React.FC = () => {
  ...
  useEffect(() => {
      ...
      const loadTeam = async () => {
          ...
          try {
              ...
              if (!ignore) {
                  setSyncMessage('Nao foi possivel sincronizar a equipe com o backend. Exibindo cache local.');
              }
          } finally {
              if (!ignore) {
  ...
  const handleDelete = (id: string) => {
      const removeMember = async () => {
          ...
          try {
              ...
              alert('Nao foi possivel remover este membro da equipe agora.');
          }
      };
  ...
  const handleSave = (e: React.FormEvent) => {
      ...
      const saveMember = async () => {
          try {
              if (editingMember) {
                  const updatedMember = await staffService.update(editingMember.id, formData);
                  setTeam(prev => prev.map(m => (m.id === editingMember.id ? updatedMember : m)));
              } else {
                  const branchId = selectedBranch?.id || formData.branchId;
                  if (!branchId) {
                      alert('Selecione uma filial antes de cadastrar um membro.');
                      return;
                  }

                  const createdMember = await staffService.create({
                      branchId,
                      ...
                      status: (formData.status as StaffStatus) || StaffStatus.ACTIVE,
              ...
              alert('Nao foi possivel salvar este membro na API agora.');
          }
      };
  ...
  const renderProfilesSection = () => (
      <div className="space-y-6">
        ...
        {loadingTeam ? (
            <Card>
                <p className="text-sm text-slate-500 dark:text-slate-400">Carregando equipe...</p>
            </Card>
        ) : (
  ...
  return (
    <div className="space-y-6">
      ...
      {activeTab === 'profiles' && !showForm && renderProfilesSection()}
